...
Esta versão separa o faturamento em duas métricas:
  - gmv_tradicional: soma total_amount de TODOS os pedidos, exceto CANCELLED
    (conta tudo que "existe" como pedido, pago ou não)
  - gmv_pago: soma total_amount só dos pedidos que já saíram do status
    UNPAID (ou seja, o pagamento já foi confirmado)

Também usa UPSERT: cada linha é identificada por (date, shop). Se a janela
de coleta normal (JANELA_COLETA_NORMAL dias) já tiver uma linha salva para
aquele dia+loja, ela é SOBRESCRITA com os valores recalculados — assim,
pedidos que estavam UNPAID e foram pagos depois entram no gmv_pago em
execuções seguintes. Uma coluna 'atualizado_em' guarda quando cada linha
foi calculada pela última vez.
...
Pedidos com status CANCELLED não entram em nenhum GMV, só na contagem
separada de cancelados.
"""
...
def buscar_detalhes_pedidos(shop, partner_id, partner_key, order_sn_list):
    ...
    IMPORTANTE: a Shopee só devolve total_amount, pay_time (e outros campos
    "extras") se pedirmos explicitamente via response_optional_fields — sem
    isso, os campos simplesmente não vêm na resposta.
    pay_time = quando o COMPRADOR pagou a Shopee (confirmação de pagamento).
    Isso é diferente de quando VOCÊ recebe o dinheiro (liberação do escrow),
    que é rastreado por um endpoint separado (ver shopee_wallet_to_sheets.py,
    ...
    for i in range(0, len(order_sn_list), 50):
        ...
        params = {
            ...
            "response_optional_fields": "total_amount,pay_time",
        }
        r = requests.get(f"{HOST}{path}", params=params, timeout=30)
        data = r.json()
        if data.get("error"):
...
def agregar_por_dia(pedidos, shop_label):
    """Agrega os pedidos em DUAS dimensões de data diferentes, e junta num só
    conjunto de linhas por (data, loja):

    1) Por DATA DE CRIAÇÃO do pedido (create_time):
       - total_orders: pedidos criados naquele dia (exceto CANCELLED)
       - gmv_tradicional: soma total_amount de tudo, exceto CANCELLED
       - unpaid_orders: quantos desses ainda estão UNPAID no momento da coleta
       - cancelled_orders: pedidos CANCELLED criados naquele dia

    2) Por DATA DE PAGAMENTO do pedido (pay_time — quando o comprador pagou):
       - gmv_pago: soma de total_amount dos pedidos PAGOS NAQUELE DIA
         (pode incluir pedidos criados em dias anteriores)
       - pedidos_pagos_no_dia: quantos pedidos foram pagos naquele dia

    Um pedido criado dia 05 mas pago dia 07, por exemplo, aparece com
    gmv_tradicional no dia 05 e gmv_pago no dia 07 — cada coluna reflete
    o momento certo daquele evento, o que é o que importa pra fluxo de caixa."""
    por_criacao = {}   # dia -> {gmv_trad, pedidos, unpaid, cancelados}
    por_pagamento = {} # dia -> {gmv_pago, pedidos_pagos}

    for p in pedidos:
        ts_criacao = p.get("create_time")
        status = p.get("order_status", "")
        valor = float(p.get("total_amount", 0) or 0)

        if ts_criacao:
            dia_criacao = datetime.fromtimestamp(ts_criacao, tz=FUSO_BR).date().isoformat()
            if dia_criacao not in por_criacao:
                por_criacao[dia_criacao] = {"gmv_trad": 0.0, "pedidos": 0, "unpaid": 0, "cancelados": 0}
            if status == "CANCELLED":
                por_criacao[dia_criacao]["cancelados"] += 1
            else:
                por_criacao[dia_criacao]["gmv_trad"] += valor
                por_criacao[dia_criacao]["pedidos"] += 1
                if status == "UNPAID":
                    por_criacao[dia_criacao]["unpaid"] += 1

        ts_pagamento = p.get("pay_time")
        if ts_pagamento and status != "CANCELLED":
            dia_pagamento = datetime.fromtimestamp(ts_pagamento, tz=FUSO_BR).date().isoformat()
            if dia_pagamento not in por_pagamento:
                por_pagamento[dia_pagamento] = {"gmv_pago": 0.0, "pedidos_pagos": 0}
            por_pagamento[dia_pagamento]["gmv_pago"] += valor
            por_pagamento[dia_pagamento]["pedidos_pagos"] += 1

    todos_dias = sorted(set(por_criacao.keys()) | set(por_pagamento.keys()))
    linhas = []
    for dia in todos_dias:
        c = por_criacao.get(dia, {"gmv_trad": 0.0, "pedidos": 0, "unpaid": 0, "cancelados": 0})
        p_ = por_pagamento.get(dia, {"gmv_pago": 0.0, "pedidos_pagos": 0})
        linhas.append([
            dia, shop_label, c["pedidos"],
            round(c["gmv_trad"], 2), round(p_["gmv_pago"], 2),
            p_["pedidos_pagos"], c["unpaid"], c["cancelados"],
        ])
    return linhas

CABECALHO = ["date", "shop", "total_orders", "gmv_tradicional", "gmv_pago", "pedidos_pagos_no_dia", "unpaid_orders", "cancelled_orders", "atualizado_em"]
...
def upsert_linhas(ws, linhas):
    ...
    Adiciona/atualiza sempre o timestamp de 'atualizado_em'."""
    dados = ws.get_all_values()
    idx_map = {}
